...
from dff.serializers.serializers_quote import QuoteListSerializer
...
class QuoteListView(ListAPIView):
    ...
    def get_queryset(self):
        try:
            ...
            queryset = Inv.objects.filter(
                company__id=user_company.id, is_quote=True)

            queryset = queryset.select_related(
                'bill_to', 'bill_to__country_code_post').select_related('status').select_related('mode').select_related('currency')
            ...
            queryset = queryset.prefetch_related(
                ...
                Prefetch('iteminv_invs', queryset=itemInvs)).prefetch_related(
                Prefetch('entry_invs', queryset=entries))
    ...
    def filter_queryset(self, queryset: QuerySet, **kwargs):
        ...
        try:
            ...
            if text_query is not None:
                # print('5050', text_query)
                queryset = queryset.filter(Q(qn__icontains=text_query) | Q(load__sn__icontains=text_query)
