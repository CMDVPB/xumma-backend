...
from ayy.models import ItemForItemCost, ItemForItemInv, ItemInv
...
class ItemForItemInvSerializer(CustomWritableNestedModelSerializer):

    # def validate_description(self, value):
    #     user = self.context['request'].user
    #     user_company = get_user_company(user)

    #     # Determine current instance id (uf) if updating
    #     uf = None
    #     if self.instance:
    #         uf = self.instance.uf
    #     elif self.initial_data and 'uf' in self.initial_data:
    #         uf = self.initial_data['uf']
    #     # For nested, sometimes data is in parent object
    #     elif hasattr(self.parent, 'initial_data') and isinstance(self.parent.initial_data, list):
    #         # Try to match nested object in list by some key if needed
    #         uf = self.parent.initial_data.get('uf', None)

    #     qs = ItemForItemInv.objects.filter(
    ...
    #             "An item with this description already exists for your company."
    #         )
    ...
    def to_internal_value(self, data):
        ...
        # print('SR5680', data_copy)

        if 'vat' in data_copy and data_copy['vat'] == '':
            data_copy['vat'] = None
        if 'code' in data_copy and data_copy['code'] == '':
            data_copy['code'] = None
        if data_copy.get('exemption_reason', '') == '':
            data_copy['exemption_reason'] = None
        if data_copy.get('is_sale', '') == '':
            data_copy['is_sale'] = True
        ...
        # print('SR5694',  data_copy['company'])

        return super(ItemForItemInvSerializer, self).to_internal_value(data_copy)
...
class ItemInvSerializer(WritableNestedModelSerializer):
    item_for_item_inv = ItemForItemInvSerializer(
        allow_null=True, context={'request': 'request'})
    item_for_item_cost = ItemForItemCostSerializer(allow_null=True)
    ...
    def to_internal_value(self, data):
        ...
        return super(ItemInvSerializer, self).to_internal_value(data)
    ...
    def save(self, **kwargs):
        ...
        possible_parents = ['load', 'trip', 'tor', 'ctr', 'inv', 'exp']
    ...
    class Meta:
        model = ItemInv
        fields = ('quantity', 'amount', 'vat', 'item_for_item_inv',
                  'item_for_item_cost', 'currency', 'discount', 'uf')
